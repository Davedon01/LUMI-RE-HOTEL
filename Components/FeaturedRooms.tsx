      ...
      <div className="mx-auto max-w-7xl">
        ...
        <div className="mb-16 flex flex-col items-end justify-between border-b border-slate-200 pb-8 md:flex-row dark:border-slate-800">
          <div className="max-w-xl">
            ...
            <p className="mt-4 text-4xl font-light text-slate-900 dark:text-slate-50 md:text-5xl">
              Curated Spaces for <span className="italic">Exquisite</span>{" "}
              Living
            </p>
          </div>
          <Link href="/rooms">
        ...
        <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
          {ROOMS.map((room) => (
            <div
              key={room.id}
              className="group relative overflow-hidden rounded-3xl bg-slate-50 transition-all hover:shadow-2xl dark:bg-slate-900/50"
            >
              {/* Image Container */}
              ...
              {/* Content */}
              <div className="p-8">
                <h3 className="mb-4 text-2xl font-light text-slate-900 dark:text-slate-50">
                  {room.name}
                </h3>
                ...
                <Link
                  href={`/rooms/${room.id}`}
                  className="flex items-center gap-2 w-full rounded-xl border border-slate-200 py-3 px-4 text-sm font-bold uppercase tracking-widest text-slate-900 transition-all hover:bg-slate-900 hover:text-white dark:border-slate-800 dark:text-slate-400 dark:hover:bg-blue-600 dark:hover:text-white"
                >
                  Explore This Suite{" "}
