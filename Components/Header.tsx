        ...
        >
          <div className="flex flex-col h-full">
            <div className="flex items-center justify-between mb-8">
              <ThemeSwitch />
              <button onClick={() => setIsOpen(false)} className="text-slate-900 dark:text-slate-50">
                <X size={28} />
              </button>
            </div>

            <nav className="flex flex-col space-y-6">
              <MobileNavLink href="rooms" onClick={() => setIsOpen(false)}>Rooms</MobileNavLink>
              <MobileNavLink href="amenities" onClick={() => setIsOpen(false)}>Amenities</MobileNavLink>
...
function NavLink({ href, children }: { href: string; children: React.ReactNode }) {
  return (
    <a href={href} className="text-sm font-medium text-slate-600 transition-colors hover:text-slate-900 dark:text-slate-300 dark:hover:text-white">
      {children}
    </a>
  );
}
